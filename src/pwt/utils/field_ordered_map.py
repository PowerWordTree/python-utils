...
核心组件:
- FieldOrderedMap[T]: 主类, 管理元素顺序与字段索引. 
...
基本示例:
    ...
    >>> fmap = FieldOrderedMap(Person, ["name", "age"])
    >>> fmap.push(Person("Alice", 30))
    >>> fmap.push(Person("Bob", 25))
    >>> fmap.pop(QueryField("name")("Alice"))
...
class QueryField:
    """
    封装查询字段名称与匹配值, 用于在 FieldOrderedMap 中按字段或按实例检索元素.
    ...
    def __call__(self, data: _Data | None) -> QueryField:
        ...
        Returns:
            QueryField: self 实例, 且 data 属性已更新.
        """
        self.data = data
        return self


class FieldOrderedMap(Generic[T]):
    """
    支持字段索引与有序访问的数据结构.
    ...
    def _make_dataproxy(self) -> type[T]:
        ...
        class DataProxy(self.data_type):
            __dataproxy_owner__: FieldOrderedMap = self
    ...
    def push(self, item: T, first: bool = False) -> None:
        ...
        if id(item) in self._items:
            raise ValueError(f"Item {item} already in container.")
